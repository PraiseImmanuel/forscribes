...
`createUpdaterArtifacts: true` only makes `tauri build` produce that `.sig`
file locally - it does **not** write `latest.json` for you. That file only
gets auto-generated by the `tauri-action` GitHub Action in CI, which this
project doesn't use, so it has to be assembled by hand every release:

```powershell
cd "C:\Users\Favour Uche\OneDrive\Documents\2025Projects\forscribe"
$sig = Get-Content "src-tauri\target\release\bundle\nsis\ForScribe_<version>_x64-setup.exe.sig" -Raw
$manifest = @{
  version  = "<version>"
  ...
  platforms = @{
    "windows-x86_64" = @{
      signature = $sig.Trim()
      url = "https://github.com/PraiseImmanuel/forscribes/releases/download/v<version>/ForScribe_<version>_x64-setup.exe"
    }
  }
} | ConvertTo-Json -Depth 4
Set-Content -Path "src-tauri\target\release\bundle\latest.json" -Value $manifest -Encoding utf8
```

The `signature` field needs the actual file *contents*, not a path or URL -
that's the part that's easy to get wrong by hand. Sanity-check it after:
